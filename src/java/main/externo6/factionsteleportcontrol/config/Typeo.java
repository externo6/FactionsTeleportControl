...
import externo6.factionsteleportcontrol.util.TypedLinkedList;
...
//		}
	...
	protected static void sanitize_AndUpdateClassMapping( Class rootClass ) {
		...
		// onEnable
		// just in case 'reload' was executed and a new FactionsTeleportControl.jar was loaded (do not use Plugin.onLoad() it's evil)
			...
			for ( int jay = 0; jay < currentFieldAnnotations.length; jay++ ) {
				...
				if ( ( Section.class.equals( annotationType ) ) || ( Option.class.equals( annotationType ) ) ) {
					...
					if ( badMods ) {
						...
						// field
						throw FactionsTeleportControl.bailOut( "bad coding: your @" + annotationType.getSimpleName()
					...
					try {
						...
						throw FactionsTeleportControl.bailOut(e, "bad coding: the field `" + field + "` doesn't have an instance, "
					...
					if ( Section.class == annotationType ) {
						...
						if ( !field.getName().startsWith( SECTION_PREFIX ) ) {
							throw FactionsTeleportControl.bailOut( "bad coding: by convention any @" + annotationType.getSimpleName()
								+ " aka sections should have their field name start with `" + SECTION_PREFIX
								+ "`. Please correct in source code this field: `" + field + "`" );
						}
						// FactionsTeleportControl.info( "Section: " + allFields[i] + "//" + currentFieldAnnotations[j] );
						...
						if ( !_Base.class.isAssignableFrom( typeOfField ) ) {
							throw FactionsTeleportControl.bailOut( "bad coding: the type of field `" + field + "` is not a subclass of `"
								+ _Base.class + "`" );
						}
...
//						FactionsTeleportControl.warn( "\n"+currentDotted);
						...
						while ( true ) {
							...
							// FactionsTeleportControl.info( currentDotted + "/" + field );
							Field existingField = Typeo.putAnyAliasDotted_to_Field( currentDotted, field );
							if ( ( null != existingField ) ) {
								FactionsTeleportControl.bailOut( "bad coding: your config option `" + currentDotted + "` in field `"
							...
							if ( currentDotted.isEmpty() ) {
								FactionsTeleportControl.bailOut( "bad coding: one of the oldAliases in field `" + field + "`\n"
									+ "should not be empty!!" );
							}
							// detect extra spaces(by mistake?) around the current old alias
							if ( !currentDotted.trim().equals( currentDotted ) ) {
								FactionsTeleportControl.bailOut( "bad coding: the old alias `" + currentDotted + "` in field `" + field
									+ "`\n" + "should not contain any extra whitespaces around it!" );
							}
						}// while
							// FactionsTeleportControl.info( "Option: " + allFields[i] + "//" + currentFieldAnnotations[j] );
					}// else we don't care
				}
			}
		}
