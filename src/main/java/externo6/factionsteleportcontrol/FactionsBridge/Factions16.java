...
import externo6.factionsteleportcontrol.util.TwoWayMapOfNonNulls;
...
public class Moo1 {
	...
	public static void main( String[] args ) throws IllegalArgumentException, IllegalAccessException, SecurityException, NoSuchFieldException {
		...
		System.out.println(b);
			...
			if ( failed ) {
				throw FactionsTeleportControlPlugin.bailOut( "failed to hook into Factions 1.6.x" );
			}
		}
	}
	...
	public void setFlag( Faction forFaction, FactionsAny.FFlag whichFlag, Boolean whatState ) {
		...
		try {
			...
			default:
				throw FactionsTeleportControlPlugin.bailOut( "setFlag, plugin author forgot to define a case to handle this flag: "
					+ whichFlag );
			...
			if ( failed ) {
				throw FactionsTeleportControlPlugin.bailOut( "failed to invoke " + mSetPeaceful );
			}
		}
	}
...
//				break;
			case POWERLOSS:
				...
				return false;
			default:
				throw FactionsTeleportControlPlugin.bailOut( "getFlag, plugin author forgot to define a case to handle this flag: "
					+ whichFlag );
				// or forgot to put a "break;"
			}
			...
			if ( null != err ) {
				throw FactionsTeleportControlPlugin.bailOut( err, "failed in getFlag");
			}
		}
//		return false;
	...
	public void addSubCommand( FCommand base, FCommand subCommand ) {
		...
		if ( null == instanceOfCmdHelp ) {
			...
			try {
				...
				if ( null != failed ) {
					throw FactionsTeleportControlPlugin.bailOut(failed, "failed to invoke " + methodUpdateHelp );
				}
			}
		}
	...
	public final void finalizeHelp() {
		if ( null == instanceOfHelpPages ) {
			throw FactionsTeleportControlPlugin.bailOut( "this should not happen, bad call order" );
		} else {
			if ( null != pageLines ) {
				instanceOfHelpPages.add( pageLines );
				pageLines = null;
	...
	public FactionsAny.ChatMode setChatMode(FPlayer forWhatPlayer, FactionsAny.ChatMode chatMode ) {
		...
		try {
			...
			if ( null == factionsChatMode_Enum ) {
				failed = true;
				throw FactionsTeleportControlPlugin.bailOut( "would never be null if .init() above failed to properly map ...");
			} else {
			...
			if ( failed ) {
				throw FactionsTeleportControlPlugin.bailOut( "failed to invoke " + mSetChatMode );
			}
			if (null == ret) {
				throw FactionsTeleportControlPlugin.bailOut( "failure within the code logic");
			}
		}
	...
	public ChatMode getChatMode(FPlayer forWhatPlayer) {
		...
		try {
			...
			if ( failed ) {
				throw FactionsTeleportControlPlugin.bailOut( "failed to invoke " + mGetChatMode );
			}
		}
