...
public abstract class Reflective {
		...
		Class<V> destinationEnum, Map<String, V> mapOfRenameSourceNameToDestEnum )
	{
		...
		try {
			...
			e1.printStackTrace();
			throw FactionsTeleportControlPlugin.bailOut( "Cannot find class " + sourceEnum );
		}
...
//							} else {
								throw FactionsTeleportControlPlugin.bailOut( "we cannot find a destination in enum class `"
...
//							}
					...
					if ( !destField.getType().equals( destinationEnum ) ) {
						...
						throw FactionsTeleportControlPlugin.bailOut( "plugin author has set the wrong field type in " + destinationEnum
					...
					if ( !ourFieldInstance.getClass().equals( destinationEnum ) ) {
						...
						throw FactionsTeleportControlPlugin.bailOut( "there was a clash: `" + destField
...
//					FactionsTeleportControl.warn( destinationMap+" mapped for enum `" + sourceEnumClass + "` `" + sourceFieldName + "` to `"
//						+ ourFieldInstance + "` in destEnum=`" + destinationEnum + "`" );
				...
				if ( failed ) {
					...
					throw FactionsTeleportControlPlugin.bailOut(error, "the plugin author forgot to define some flags in " + destinationEnum
						+ " for " + eachSourceField );
				}
			}
		}
