...
import externo6.factionsteleportcontrol.extras.BailingOutException;
...
public abstract class FactionsTeleportControlPlugin extends JavaPlugin {
...
//		System.out.println("---------- onLoad() ---------");//yep not even called by PlugMan
	...
	public void onEnable() {
		if ( !didNotOverrideOnLoad ) {
			throw bailOut( "You should not override & use onLoad() because it may cause problems, please just use onEnable()"
				...
				"onLoad() method after having unloaded the Plugin class. Maybe try using /f reloadftc instead." );
		}
	}
	...
	public static RuntimeException bailOut( Throwable cause, String msg ) {
		severe( cause, msg );
		throw new BailingOutException( msg, cause );// disableSelf( FactionsTeleportControl.instance, true );
	}
	
	
	public static void warn( String logInfoMsg ) {
		tellConsole( ChatColor.GOLD + "[FTC] " + ChatColor.DARK_RED + "[WARNING]:" + ChatColor.RESET
			+ logInfoMsg );// they are logged with [INFO] level
	}
	
	
	public static void info( String logInfoMsg ) {
		tellConsole( ChatColor.GOLD + "[FTC] " + ChatColor.RESET + logInfoMsg );// they are logged with
																									// [INFO] level
	}
	
	
	public static void severe( String logInfoMsg ) {
		FactionsTeleportControl.log.severe( "[FTC] " + logInfoMsg );// allowed so that [SEVERE] appears
		tellConsole( ChatColor.RED + "[FTC] " + ChatColor.DARK_PURPLE + logInfoMsg );
	}
	...
	public static void severe( Throwable cause, String logInfoMsg ) {
		...
		String msg = "[FTC] " + logInfoMsg;
		if ( null == cause ) {
			FactionsTeleportControl.log.log( Level.SEVERE, msg);// allowed so that [SEVERE] appears
		} else {
			FactionsTeleportControl.log.log( Level.SEVERE, msg, cause );// allowed so that [SEVERE] appears
		}
		tellConsole( ChatColor.RED + "[FTC] " + ChatColor.DARK_PURPLE + logInfoMsg );
	}
	...
	// public static RuntimeException disableSelf( FactionsTeleportControl fpInstance, boolean forceStop ) {
	// fpInstance.disableSelf();
	// if ( forceStop ) {
	// // FactionsTeleportControl.log.log( Level.INFO, )
	// throw new RuntimeException( FactionsTeleportControl.FP_TAG_IN_LOGS
