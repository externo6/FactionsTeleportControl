...
public abstract class FactionsBase implements FactionsAny {
	...
	public FactionsAny.Relation getRelationBetween( RelationParticipator one, RelationParticipator two ) {
		...
		try {
			...
			if ( null == ret ) {
				FactionsTeleportControlPlugin.severe( "impossible to be null here, because it would've errored on .init(),"
			...
			if ( ( failed ) || ( null == ret ) ) {
				throw FactionsTeleportControlPlugin.bailOut( "failed to invoke " + mGetRelationTo );
			}
		}
	...
	public Relation getRole( RelationParticipator one ) {
		...
		try {
			...
			if ( null == ret ) {
				if ( null == isReturn ) {
					FactionsTeleportControlPlugin.severe( "the source getRole() returned null, must be Faction's fault :)) Actually this " +
					...
					FactionsTeleportControlPlugin.severe( "there was no mapping for `" + isReturn + "` => bridging failed" );
				}
			}
		} catch ( IllegalAccessException e ) {
			error = e;
			...
			if ( ( null != error ) || ( null == ret ) ) {
				throw FactionsTeleportControlPlugin.bailOut( error, "failed to invoke " + mGetRole );
			}
		}
		return ret;// actually reached
	}
	...
	public void setSenderMustBeFactionAdmin( FCommand fCommandInstance, boolean flagState ) {
		...
		try {
			...
			if ( failed ) {
				throw FactionsTeleportControlPlugin.bailOut( "failed to set field " + fSenderMustBe_FactionAdminLeader );
			}
		}
	}
}