...
public class Factions17 extends FactionsBase implements FactionsAny {
	...
	protected Factions17( ) {
		...
		try {
			...
			if ( failed ) {
				throw FactionsTeleportControlPlugin.bailOut( "failed to hook into Factions 1.7.x" );
			}
		}
	}
	...
	public void setFlag( Faction forFaction, FactionsAny.FFlag whichFlag, Boolean whatState ) {
		...
		try {
			...
			if ( null != failed ) {
				throw FactionsTeleportControlPlugin.bailOut(failed, "failed to invoke " + mSetFlag );
			}
		}
	}
	...
	public boolean getFlag( Faction forFaction, FactionsAny.FFlag whichFlag ) {
		...
		try {
			...
			if ( null != failed ) {
				throw FactionsTeleportControlPlugin.bailOut( failed, "failed to invoke " + mGetFlag );
			}
		}
